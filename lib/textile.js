...
(function (root, name, factory) {
  ...
  TextileConverter.prototype = {
    ...
    makeTag: function (tagName,attr,klass,style) {
  ...
  phraseTypes.push(function (text) {
    var out = [], m, pre, post, attr, klass, style, img, tag, bit;
    ...
    while (m = re.image.exec(text)) {
      ...
      img = this.makeTag('img',attr,klass,style).single;

      if (m[6]) {
        tag = this.makeTag('a',{href:m[6]});
  ...
  phraseTypes.push(function (text) {
    ...
    while (m = re.iframe.exec(text)) {
      ...
      tag = this.makeTag('iframe',attr);
      bit = [text.slice(0,m.index)+pre,tag.open,' ',tag.close,post];
      out = this.joinPhraseBits(out,bit,out.length);
      text = text.slice(re.iframe.lastIndex);
